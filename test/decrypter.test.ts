...
describe('Decrypter', () => {
    it('should decrypt stuff', async () => {
        ...
        assert.deepStrictEqual(
            seedqb64b,
            new Matter({ raw: seed, code: MtrDex.Ed25519_Seed }).qb64b
        );
        ...
        const salter = new Salter({ raw: raw, code: MtrDex.Salt_128 });
        assert.equal(salter.code, MtrDex.Salt_128);
        const saltqb64 = salter.qb64;
        const saltqb64b = salter.qb64b;
        assert.deepStrictEqual(saltqb64, '0AA2CGQNobs5jXCNoMATSody');

        // also works for Matter
        assert.deepStrictEqual(
            saltqb64b,
            new Matter({ raw: raw, code: MtrDex.Salt_128 }).qb64b
        );
        ...
        // create encrypter
        const encrypter = new Encrypter({ raw: pubkey });
        ...
        const seedcipher = encrypter.encrypt(seedqb64b);
        ...
        const saltcipher = encrypter.encrypt(saltqb64b);
